...
> *   **Совместимость с протоколом старых модулей Transformer** (двухэтапная загрузка).
> *   Gotenberg 8.x.
...
В отличие от многих примеров в сети, этот модуль требует **двухэтапного** процесса загрузки результата:
1.  **POST Upload**: Загрузка контента файла с параметрами `file_name`, `last_part=y`.
2.  **POST Finish**: Команда завершения с параметрами `finish=y`, `result[files][]`.

Если вы не отправите команду Finish, Битрикс примет файл (200 OK), но задача конвертации навсегда останется в статусе "В процессе" (висящий прогресс-бар).
